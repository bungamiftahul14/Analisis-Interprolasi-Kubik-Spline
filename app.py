...
st.markdown(
    ...
    <div style="text-align:center;">
        <h1>🌸 Interpolasi Cubic Spline 🌸</h1>
        <p style="font-size:18px; color:#c2185b;">
            Haii! Selamat datang di website interpolasi paling cute 💗<br>
...
if st.button("💖 Hitung Interpolasi"):
    if (
        ...
        st.error("❌ Semua input wajib diisi yaa 💗")
        st.stop()
    ...
    if len(x) < 3:
        st.error("❌ Minimal 3 titik data diperlukan untuk cubic spline 🌸")
        st.stop()
    ...
    if not (x.min() <= x_pred <= x.max()):
        st.error(f"❌ Nilai X prediksi harus di antara {x.min()} dan {x.max()} 💗")
        st.stop()
...
st.markdown(
    """
    <p class="footer">Dibuat oleh Bunga, Aira dan Uti 💗</p>
    """,
    unsafe_allow_html=True
)
